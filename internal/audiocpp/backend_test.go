...
func TestBackendContract_Version(t *testing.T) {
	t.Skip("version endpoint not implemented in fake child")
}

func TestBackendContract_Capabilities(t *testing.T) {
	cfg := pkgConfigForTest(t)
	if cfg.Backend == "" {
		t.Log("no backend configured, using default")
	}
	if cfg.Threads < 1 {
		t.Error("expected at least 1 thread")
	}

	expected := []string{"tts", "asr", "voice_clone", "voice_conversion",
